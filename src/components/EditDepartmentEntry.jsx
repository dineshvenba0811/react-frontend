...
function EditEmployeeEntry() {
    ...
    useEffect(() => {
        fetch(`http://localhost:8083/api/departments?id=${id}`,{
            method:'GET',
            headers:{
            ...
            .then(response => setDepartmentDetailsResults(response))
            .catch(error => console.log(error))
    },[])

    const setDepartmentDetailsResults = response => {
        setDepartmentName(response[0].departmentName)
        setDepartmentCode(response[0].departmentCode)
        setDepartmentDescription(response[0].departmentDescription)
    }
    const handleSubmit = events => {
        fetch(`http://localhost:8083/api/departments/update?id=${id}`,{
            method:'PUT',
            headers:{
                'Content-Type':'application/json'
            },
